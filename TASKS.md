...
- [x] `tsc` and lint clean
- [x] `tsc` and lint clean
- [x] **Every organization is seeded with the default lease template when it is created** — both paths: `createOrganizationForUser` and `POST /api/auth/register`. Inside the org-creation transaction, taking the transaction client, so a template can never be committed for an organization whose own insert rolls back
- [x] `seedDefaultLeaseTemplate` placed in `lib/lease-template-starters.ts` to avoid a real `lease-templates ↔ organizations` import cycle; `DEFAULT_TEMPLATE_NAME` re-exported so no caller changed. `ensureDefaultLeaseTemplate` kept as the backstop for pre-existing organizations
- [x] **Verified both paths against the database**: a fresh org from `createOrganizationForUser` has exactly 1 template (`Standard tenancy agreement`, default, 4,309-char body with live `{{placeholders}}`); a fresh registration has roles `Owner, Tenant` **and** the template
- [x] **Verified the whole chain on a brand-new organization**: register → property → unit → tenant → lease → `lease.created` published → rendered from the *seeded* template (no `ensureDefaultLeaseTemplate` fallback) → `document.stored` → `contract-L-K2PY5.pdf` filed, 102,778 bytes. Then the **Generate button** on the same lease: prepares the parameters, publishes `lease.created`, answers `202 {queued, fileName, missing}`, and the contract is re-filed. All probe data removed afterwards
- [x] **Routing keys are configuration, not literals.** `LEASE_CREATED_ROUTING_KEY` and `DOCUMENT_STORED_ROUTING_KEY` added to `.env`/`.env.example` (with `DOCUMENTS_QUEUE`, previously defaulted but never written down), alongside a table pairing each with the `document-worker` setting it must match
- [x] The event's **identity** stays literal in code so `DomainEvent` keeps type-checking payloads; only the **wire key** is configurable, via `ROUTING_KEY`/`CONSUMED_ROUTING_KEY`. `publishEvent`, `declareEventTopology` and the worker all route through them — `grep` confirms no dotted key is spelled out at any publish or bind site
- [x] **Verified `document.stored` is genuinely listened to** (the question that prompted this): `jarvis.events --[document.stored]--> JARVIS_DOCUMENTS_QUEUE` with `consumers=1`, and the worker's startup line now names the key as well as the queue, so a misconfigured key is distinguishable from a quiet broker
- [x] **Both directions proven against the live broker**: `DOCUMENT_STORED_ROUTING_KEY=document.stored.probe` bound the queue to that key; `LEASE_CREATED_ROUTING_KEY=lease.created.pubprobe` sent the message to a queue bound to *that* key while `DOCUMENT_WORKER_QUEUE` stayed empty. Probe queues and the one junk object removed afterwards
- [x] Re-verified end to end afterwards: 2 leases backfilled, **19 leases / 19 contracts**, and every stored object confirmed present in MinIO
...
### Not done
- [ ] `PaymentAccount` and `FileAsset` rows are not part of this backup — payment destinations and uploaded documents/photos have no sheet, on either the export or the import side

## Phase 87 — Organization backup restore (the re-import side of Phase 86)

- [x] `lib/organization-import.ts` — `parseOrganizationBackup(buffer)` reads and validates all five sheets **before anything is written**: required fields, valid enum values, no duplicate ids within a sheet, and every foreign key column (`Unit.propertyId`, `Lease.unitId`/`membershipId`/`renewedFromId`, `Payment.leaseId`) resolves to a row that actually exists elsewhere in the same file. Returns every problem it finds at once, not just the first
- [x] `importOrganizationBackup(organizationId, data)` runs as **one Prisma transaction** (60s timeout — a few hundred rows across five tables, each its own round trip, would blow past the 5s interactive-transaction default) that trusts the validation completely: it just builds an old-id → new-id `Map` per sheet while creating rows, in the phased order Phase 86 wrote down (Properties → Units → Memberships → Leases → Payments), and rewrites each child's foreign keys through that map. A lease's own `renewedFromId` gets a second pass once every lease in the file has an id, since the lease it renewed from can appear later in the sheet
- [x] `POST /api/organizations/import` — Owner-only (checked the same way `DELETE /api/organizations/[id]` does), and **refuses unless the active organization is empty** (`propertyCount === 0 && membershipCount <= 1` — the `<= 1` allows exactly the owner performing the restore). Multipart upload, 2MB cap reusing `MAX_IMPORT_BYTES`; a validation failure returns 422 with every issue found, and nothing is written
- [x] **The Memberships sheet has no Invoice or Roles sheet to lean on, so both are reconstructed deterministically rather than looked up**: a lease's invoice is rebuilt as `{ amount: leaseAmount, dueDate: startDate }` — verified against `insertLease`/`updateLease`, which set exactly those two fields and nothing else, so no data is actually lost by skipping an Invoice sheet even for a lease with zero payments. A role is found-by-name-or-created in the target org, exactly `ensureRole`'s existing logic
- [x] **A membership's `User` is found by email-then-phone before a new one is created** — same reasoning as `createTenant`: the realistic restore path is "I deleted my org, its members' User rows survived (Phase 82: users are never touched by an org delete), and I'm reconnecting them to a new org," so reusing the match is required, not just a nicety — creating a fresh row would 409 on the unique email/phone constraint the surviving account already holds
- [x] **The importing owner's own membership is reused, never duplicated**: every membership row checks `(userId, organizationId)` before creating, and the Owner row in the sheet is that owner's own — this is what makes "restore into the org I just created" work at all, since that owner already has exactly one membership there
- [x] `components/organization-import-dialog.tsx` — file picker, upload, result. No per-row review like the units/tenants `ImportDialog`: the five sheets reference each other, so accepting some rows and rejecting others would leave a lease pointing at a unit that was never created. One file in, either a full restore or a full list of what's wrong
- [x] `components/profile/organization-data-actions.tsx` — Export and Restore live side by side on the same card action slot; Restore greys out (not hides) once the organization has any data, `title` explaining why, matching `RowAction.disabled` everywhere else
- [x] **Verified against the real intended flow, not just a script fixture**: registered an owner, built a property/unit/tenant/lease/payment through the actual UI-facing API endpoints, exported the backup, **deleted the source organization** (`DELETE /api/organizations/[id]`, the real cascade), created a fresh organization for the same owner, and restored the backup into it. Confirmed by direct query: exactly one `User` row per person (no duplicates), the owner's pre-existing membership was reused (its `createdAt` is when the *new* org was made, not the restore), the tenant's membership carries its *original* `createdAt` from the source org, and every FK — Unit→Property, Lease→Unit/Membership, Invoice→Lease, Payment→Invoice — resolves correctly. Also verified both guards: restoring into a non-empty organization → 409, and a file with a broken cross-reference → 422 naming the exact row, with **nothing** written despite an otherwise-valid Properties row in the same file. `tsc --noEmit` and lint clean. All test data removed afterward
- [x] **UI could not be visually verified this session** — the Browser pane failed to navigate to the running dev server across a full server/tab restart, while `curl` against the same port succeeded throughout, pointing at a session-level tooling issue rather than an app bug. The two new client components follow the exact structure of `ExportButton`/`ImportDialog`, which were visually confirmed working in Phase 86's session

## Phase 83 — Generating a contract streams, so a failure is visible

- [x] **`POST /api/leases/[id]/contract` renders inline and streams SSE** instead of publishing an event and returning 202. The queue is still right for the automatic path — a lease must not wait on a browser, and a storage outage must not fail a signing — but it was wrong *here*, where a person is watching the button. A 202 could report that the message was accepted and nothing else: a missing browser, a refused upload and a template that would not resolve all looked identical to success
- [x] `generateAndStoreContract` takes an optional **`onStep` reporter**, called as each phase begins. The worker passes nothing and logs only the outcome; the route passes one that writes each step down the response. Three named phases — `template`, `render`, `store` — because "render" and "store" fail for entirely different reasons and want entirely different fixes, so *which* one stopped is half the diagnosis
- [x] **Server-sent events, not NDJSON**: `text/event-stream` is the content type proxies and dev servers reliably refuse to buffer, and a progress stream that arrives in one lump at the end is not a progress stream. `Cache-Control: no-store, no-transform` (the `no-transform` matters as much — a proxy that gzips will also buffer) plus `X-Accel-Buffering: no`. Read with `fetch` + a reader rather than `EventSource`, which cannot POST
- [x] Auth and the lease lookup happen **before the stream opens**, so "not found" stays an ordinary 404 the client branches on rather than an error event inside a 200
- [x] **`lib/contract-steps.ts` — a new client-safe module.** The Contract tab is a client component, and importing one label constant from `lib/contracts.ts` dragged Playwright into the browser bundle: `npm run build` failed with **144 errors**, all `playwright-core [Client Component Browser]`. Same split as `lib/document-options.ts`: the constants both sides agree on, with no dependencies of their own
- [x] The tab shows a **live step log** that stays after the run — spinner, then green checks, and a red marker on the phase that stopped — and the failure panel prints **the server's own message**, not a rewritten one. A rewritten message cannot name the file, the bucket or the missing binary
- [x] Verified the success path in the browser: three steps ticked through, the row appeared with no refresh, and the toast read `contract-L-OW5LY.pdf filed — 7 fields had no data`
- [x] **Verified a real failure**, by pointing `STORAGE_SECRET_ACCESS_KEY` at a wrong value and restarting: template and render went green, **store went red**, and the panel printed MinIO's own words — *"The request signature we calculated does not match the signature you provided. Check your key and signing method."* The button became "Try again". Under the old flow this was an empty tab and "refresh in a moment", forever. `.env` restored from a backup and verified byte-identical afterwards
- [x] `tsc`, lint (0 errors) and a clean `npm run build`
...
- [x] ~~**`deleteDocument` deletes the row before the object**, so a storage failure orphans the file... Object-then-row is the fix~~ — **re-examined 2026-09-17 and it isn't a bug.** This note contradicted a decision already on record: the 2026-08-24 entry in `plan.md` chose row-then-object *for deletes specifically*, on purpose — "a row whose object is gone renders as a broken document, so the row goes first." `lib/documents.ts`'s `deleteDocument` (line 539) still deletes the row then calls `deleteObject`, exactly as designed, and every object key is a fresh `randomUUID()` (`buildObjectKey`), so a regenerated contract never even collides with the old one. The accepted cost is a storage-side orphan, not a broken reference in the UI — which is the tradeoff, not a defect in it. The reaper for those orphans (Phase 67) is still genuinely open; the *ordering* was never the problem
- [x] ~~No cancel: once the stream starts there is no way to abandon it from the UI~~ — **moot as of Phase 89.** The stream this refers to doesn't exist any more: rendering moved to `document-worker`, and `POST /api/leases/[id]/contract` now only builds a plan and publishes — nothing runs in this process long enough to need cancelling. See `app/api/leases/[id]/contract/route.ts`'s own doc comment
...
- [x] ~~No cancel, still: dismissing the progress toast hides it but does not stop the render~~ — **moot as of Phase 89, same as the Phase 83 note above.** The toast this refers to doesn't track a render any more — `ContractTab.handleGenerate` now resolves in about the time a `POST` takes (build a plan, publish, done), with nothing left running client-side to cancel. Confirmed against the live component: 2026-09-17
...
- [x] ~~The authenticated button click was **not** exercised in a browser~~ — **done 2026-09-17.** Registered a throwaway owner account through the real UI (`zzcontractgenprobe@example.com`, `emailVerifiedAt` set directly to clear the Phase 64 gate the way Phase 62's own testing did), clicked through Add property → Add unit → Add tenant → Create lease, opened the fresh lease's Contract tab — empty, Generate button showing — and clicked it for real. Toast rendered *"contract-L-MI9QE.pdf is being generated / 7 fields had no data. It will appear here once the document worker has filed it."*, no console errors. `DOCUMENT_WORKER_QUEUE` held 2 messages afterwards (the automatic publish from lease creation plus this manual one), confirmed live over AMQP rather than the management API's polled stats, which lag several seconds behind. Throwaway org, user and both queue messages removed afterwards
...
- [x] ~~`contracts.generate.dead` still holds 9 messages from the old pipeline, now unbound.~~ — **deleted in Phase 91** (9 messages, all inspected first), the same phase that retired the queue entirely as part of the broker naming convention. This note was never updated when that landed; corrected 2026-09-17
...
### Not done
- [x] ~~**`contracts.generate.dead` still holds 9 messages** and I did not delete it~~ — **deleted in Phase 91**, the very next phase: all 9 inspected, none recoverable, queue retired outright. Corrected 2026-09-17 rather than left to keep saying otherwise
- [ ] **No DLQ drain or alert anywhere** — re-checked 2026-09-17: `DOCUMENT_WORKER_QUEUE_DEAD` is currently empty (the 1 test message noted here is gone — someone or something cleared it since), but that's incidental. Nothing watches any `_DEAD` queue, and no drain tool exists beyond the manual `curl`/`purgeQueue` calls this session and Phase 91 both used by hand. Still open, still spread across two services
- [x] ~~`lease.renewed` still generates no contract~~ — **done 2026-09-17.** `runAutoRenewals` never called `buildContractPlan`; a renewal reused `insertLease` same as always, but nothing queued the new lease's contract, so it sat empty until someone noticed and clicked Generate by hand. New `queueContractsForRenewals` in `lib/lease-renewal.ts` closes it — same `buildContractPlan` + `publishEvent("lease.created", …)` pair the lease route already runs, called from both `/leases` and `/dashboard` inside the same `after()` that already announces the renewal by email. One function rather than duplicating the pair into both pages, which is the exact gap Phase 66 predicted: "a third caller of `runAutoRenewals` would have to remember the `after()` line." Verified against the real broker, not just reasoned about: built a throwaway org with an auto-renewing unit and a lease ended yesterday, ran `runAutoRenewals(orgId, { force: true })`, confirmed `DOCUMENT_WORKER_QUEUE`'s depth rose by exactly one over a live AMQP `checkQueue` call (immune to the management API's polling lag, which cost the first two attempts at this measurement a false negative). Throwaway org and the test message both removed after. `tsc` and lint clean (2 pre-existing unused-import warnings in `dashboard/page.tsx`, untouched by this change)
- [ ] The Generate button still cannot report a failed render (the `DocumentJob` table) — unchanged from Phase 89, and the one item from this whole cluster still worth a real decision. See the note below
...
## Phase 95 — Triage: phases 83, 88, 89, 90 re-checked against the current pipeline

Asked to go back through the contract-pipeline phases' "Not done" lists and, for each item, either fix it or confirm it's genuinely resolved and mark it so — not leave stale notes standing. Several turned out to describe code that Phase 89 had already replaced.

- [x] **`deleteDocument`'s delete order** (Phase 83) re-examined and found not to be a bug — it matches a 2026-08-24 architecture decision on purpose. Corrected in place rather than fixed, since there was nothing to fix
- [x] **Both "no cancel" notes** (Phase 83, Phase 88) marked moot — they describe the SSE-streaming Generate button and its progress toast, both removed when Phase 89 moved rendering to `document-worker`. Nothing in the current click-to-queue flow runs long enough to need cancelling
- [x] **`contracts.generate.dead`**, flagged as still-holding-9-messages in both Phase 89 and Phase 90, was actually deleted the very next phase (91) — neither note had been corrected when that landed. Both now are
- [x] **The Generate button's browser click**, never exercised (Phase 89), was today — see that phase's entry for the full run. First real click since the queue-only rewrite
- [x] **`lease.renewed` generates no contract** (Phase 90, and predicted structurally back in Phase 66) — genuinely still true, and fixed: `queueContractsForRenewals` in `lib/lease-renewal.ts`, wired into both callers of `runAutoRenewals`. See Phase 90's entry
- [ ] **Left alone, deliberately: the `DocumentJob` / job-status table.** This is the one item every phase since 83 has deferred for the same reason — it needs a status row both this repo *and* the separate `document-worker` NestJS service can write to and read, which means a new migration here, a new outbound event (or an HTTP callback) from there, and UI polling to replace the toast's current fire-and-forget optimism. That's a cross-repo feature, not a fix, and four phases in a row choosing not to build it on the side of an unrelated task is a pattern worth respecting rather than overriding on the fifth. Flagged for a decision rather than built
- [ ] **Left alone: DLQ drain/alerting.** No monitoring integration exists anywhere in this app (no Slack, no PagerDuty, nothing that watches a queue depth). A manual drain script mirroring `backfill:contracts`'s shape is a small, contained addition if wanted; real alerting is infrastructure this app doesn't have anywhere else, so adding it just for one queue would be a new category of thing, not a bug fix
- [x] `tsc` clean; lint clean on every touched file (2 pre-existing unused-import warnings in `dashboard/page.tsx`, not from this change)
...
- [x] **Consumers are kicked off, not awaited, in `register`** — a caught rejection does not cover a *hang*, and an unreachable broker on a cold deploy would have held the whole site behind a ~75s TCP connect. Invisible locally, where the broker answers instantly
- [x] `prisma.$disconnect()` moved out of the consumer's stop function into the CLI shim — it is the shared singleton, and in-process it would have closed the web app's pool
- [x] **Verified against a production build**, which is what Railway actually runs — not `next dev`: `npm run build` succeeded (and did *not* try to reach RabbitMQ, confirming `register` is a server-start hook, not a build one), `next start` logged `✓ Ready` **before** the consumer lines (proving the non-blocking change), both queues went 0 → 1 consumer, and a published `document.stored` was received, refused and acked
- [x] Confirmed `serverExternalPackages: ["amqplib"]` in `next.config.ts` already covers the production bundle — without it the socket layer breaks when bundled
- [ ] Sweep MinIO for contract objects with no `FileAsset` row (orphans from the two deleted leases, and any future refusal)
- [x] Migration `unit_renewal_defaults`: `Unit.autoRenew` defaults true, `minTenureMonths` defaults 6, existing units backfilled (null/0 tenure → 6, autoRenew → true); unit form and `units-schemas.ts` follow

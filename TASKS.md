...
- [x] **Bug found and fixed while testing**: `optionalText` in `lib/units-schemas.ts` was `.optional()` but transformed blanks to `null`, so the schema couldn't re-parse its own output — every imported row with a blank Floor/Block failed with "expected string, received null". Now `.nullish()`. The same shape remains in four other schema files

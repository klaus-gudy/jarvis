...
## Phase 42 — File storage: MinIO locally, Cloudflare R2 in production

- [x] **MinIO added to `docker-compose.yml`** (`jarvis-storage`, API on 9010, console on 9011 — 9000/9001 are taken by another project on this machine), plus a one-shot `minio-init` container that creates the `jarvis-files` bucket and sets it private, so a clean clone gets working storage from `docker compose up -d` alone
- [x] **`lib/storage.ts`** — the whole S3 surface: client singleton (same hot-reload guard as `lib/prisma.ts`), key generation, presigned PUT/GET, `HeadObject`, delete. `forcePathStyle: true` serves both providers — MinIO on localhost has no wildcard DNS for `bucket.localhost`, and R2 accepts path style. Missing env vars are reported **by name**, since on a fresh deploy it's usually one typo
- [x] **`Attachment` model** with five nullable owner FKs (property / unit / lease / membership / payment) and a hand-written `Attachment_one_owner` CHECK constraint in the migration — Prisma can't express "exactly one of these", and without it a row could belong to two owners at once or to none
- [x] **Upload is presigned-PUT direct to the bucket**: `POST /api/attachments/presign` issues a 5-minute URL, the browser PUTs the bytes, `POST /api/attachments` records the row *after* confirming the object exists. Size and content type are read off the object with `HeadObject`, not taken from the request
- [x] **The size cap is enforced after the upload, not during it** — a presigned PUT can't carry a byte limit a browser will honour (`Content-Length` isn't settable from `fetch`), so an object over 10 MB is **deleted** rather than recorded. The browser checks first so the user hears about it before waiting; the server check is the one that binds
- [x] `GET /api/attachments/[id]` re-checks the org and 302s to a presigned URL with `Cache-Control: no-store` — the bucket stays private and no link outlives its permission check. `?download=1` switches `Content-Disposition` to `attachment`, with RFC 5987 encoding so non-ASCII filenames survive
- [x] **`components/attachments/attachments-card.tsx`** is generic over the owner, so units, properties, tenants and payments are a drop-in. Wired into the lease detail page's **Contract tab**, which had been a placeholder reading "Contract documents … will live here"
- [x] The PUT is the one request that goes through `XMLHttpRequest` — `fetch` can't report upload progress, and a 10 MB scan over a phone connection with no feedback reads as a hung dialog
- [x] `lib/attachment-types.ts` holds the size cap, the closed accepted-type list and the pure helpers, with no Prisma or AWS import — the upload card needs them as *runtime* values, and the `pg` → `require('dns')` build failure is the same trap already documented for `lib/invoice-types.ts` and `lib/search-types.ts`
- [x] Verified end-to-end against local MinIO: a PDF and a PNG uploaded from the browser, both objects landing under `org/<orgId>/lease/<leaseId>/…` with the sizes read back off the bucket (69 B and 70 B, matching the bytes sent), thumbnail rendering through the private-bucket redirect, download returning `Content-Disposition: attachment` with the original filename, and **delete removing the row and the object together** — bucket confirmed empty afterwards
- [x] **Multi-tenancy checked, not assumed**: a row planted in another org returned **404 on both GET and DELETE** and survived the attempt; presigning against an owner id from another org returned 404. Owner lookups are scoped through every relation, the way `getLease` already is
- [x] Migrations verified against a **throwaway empty Postgres**, not the already-migrated dev DB: all 15 applied cleanly and `Attachment_one_owner` materialized — the same check the Railway deploy work used in Phase 34
- [x] README gained a **File storage** section: local ports and credentials, the R2 variables for Railway, and the bucket **CORS policy** — uploads go browser→bucket, so without it every upload fails with an opaque network error

## Phase 43 — Document titles, slot checklists, and in-page preview

- [x] **`lib/attachment-slots.ts`** — the named documents each record is expected to hold: lease → signed contract / addenda / handover checklist, property → photos / title deed / floor plans, unit → photos / condition report, member → **NIDA card** / passport photo / proof of income, payment → receipt. Code constants, matching `lib/unit-options.ts`
- [x] **Every slot renders whether or not anything is in it**, reading "Not uploaded" under its own heading and description — the point of the request: a tenant's missing NIDA is information, and an empty "no files" card hides it
- [x] `Attachment` gained **`slotKey`** (which named document this is) and **`title`** (this file's own label). The **key** is stored rather than the label, so renaming "Employment letter" → "Proof of income" in config renames it on files uploaded years earlier
- [x] Display name resolves **title → slot label → filename**. All three are legitimately the best name in different cases: a photo in a multi-file slot needs its own title, a single-slot file is fully described by the slot, and a row from before slots existed has only its filename
- [x] **Files whose slot no longer exists fall into "Other documents"** rather than disappearing — deleting a slot from the config must never make its files unreachable, only unfiled. Verified with the pre-slots row already in the database, which now renders there
- [x] An **unknown slot key is refused** at both presign and register — checked in `lib/attachments.ts`, where the owner type and the slot list are both in hand, not in the zod schema which only sees one. A key no slot defines would otherwise render under no heading at all
- [x] Upload is now a **form**: document type (defaulting to the slot whose button was pressed), title, file. A title is **required only when no slot is chosen** — that is the one case where nothing else would name the file. Verified: submitting an "Other document" with no title is refused with "Give this document a title, or pick a document type"
- [x] **Eye button opens a preview modal** — image inline, PDF in an iframe, text/CSV as text, and an honest "this file type can't be previewed here" for Word and Excel. The text branch fetches in the **click handler**, not an effect, since the codebase lints against set-state-in-effect
- [x] **Wired across three surfaces**: lease → Contract tab, property → new **Documents** tab, member → new **Documents** tab (reached from both Tenants and Users). Upload and delete now revalidate the paths that owner type actually appears on, via `OWNER_REVALIDATE_PATHS`, rather than always `/leases`
- [x] **Replacing a single-file slot uploads first and deletes after** — the obvious order leaves a window where the record has no document at all, and if the upload then fails that is where it stays
- [x] Verified in the browser across all three surfaces: slots listed before any upload; a contract uploaded with the title "Hassan Said — Z4 tenancy agreement" and the button switching to **Replace**; a property photo previewing inline at full size; a CSV rendering its rows in the modal; a `.docx` giving the download-only fallback. Test files removed from both the database and the bucket afterwards

### Not done
- [ ] **The PDF branch of the preview is unconfirmed visually.** The content is served correctly (`200`, `application/pdf`, `Content-Disposition: inline`, no framing headers) and the iframe navigates cross-origin as expected, but the automation browser is configured to download PDFs rather than display them, so the render itself was never seen. Worth one manual check in an ordinary browser.
- [ ] **Slots are the same for every organization.** A landlord who needs a business licence and one who doesn't get the same list. Becomes a model of the same shape when that matters, with `slotKey` as the join.
- [ ] **Units and payments have slots defined but no UI.** Units live in a dialog rather than a page, and the payments table deliberately has no detail view — both need a surface before `condition-report` and `receipt` are reachable.
- [ ] **No image resizing on upload.** A 9 MB phone photo is stored at 9 MB and served at 9 MB to a thumbnail. `sharp` is already a dependency; downscaling in the browser before the PUT would cut both storage and egress.
- [ ] **`image/heic` is deliberately rejected** — iPhones shoot it by default and no browser renders it in an `<img>`, so accepting it would mean storing files whose preview is permanently broken. Converting on upload is the real fix.
- [ ] **An upload that lands but fails to register leaves a stray object.** Chosen over the alternative (a row describing a file that never arrived), but nothing sweeps them yet — a job listing each org prefix and deleting keys with no row would close it.
- [ ] No virus scanning, and no limit on how many files one record can hold.

### Not done (Phase 41 and earlier)

...
- [x] **`components/skeletons.tsx`** + a `loading.tsx` for all 12 routes under `app/(app)/` — there were none, so every navigation sat on the old page until data landed. Each placeholder is built from the *real* component's wrapper (`DataTableSkeleton` uses the same `<Card className="gap-4 p-4">` and `<Table>` as `DataTable`), so the swap doesn't shift the page. Bar widths cycle a fixed list rather than `Math.random()`, which would differ across hydration

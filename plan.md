...
| Date | Decision | Why |
|------|----------|-----|
| 2026-09-25 | **Member page "SMS alerts" tab reads notifier's audit trail live** (`GET /api/members/[id]/sms` → notifier `GET /api/v1/notifications`, server-only `NOTIFIER_API_URL`) | Jarvis stores no SMS; notifier already keeps a row per message with status/date/paging filters. Rows are matched by the member's phone in `255…` form (`toInternationalTzPhone`) because notifier has **no organization column**, so a number shared across orgs shows every org's texts. Fetched client-side on tab open so a notifier outage never breaks the member page. |
